...
/*
	ProcessA creates a shared memory segment
	ProcessA attaches a pointer to the shared memory segment created
	.. then it writes an integer value and a character value to it
	.. it prints the character value for the integer number of times
	.. then it waits on ProcessB to update the value of integer and the character
	.. .. this will be indicated by the flag value being set to 1 by ProcessB 
	.. after the same printing operation is done, ProcessA will detach and delete the shared memory segment
...
{  
 ...
 putchar('\n');
	...
	// signal processB that ProcessA is ready for next round, which will be printed below
	ctrl->flag = 2;
